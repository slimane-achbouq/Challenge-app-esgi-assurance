...
export default {
  ...
  async created() {
    ...
    const request = await axios.get(`${import.meta.env.VITE_API_URL}/payment/${this.id}`, {
      headers: {
    ...
    if (!request.data) {
      const request2 = await axios.post(`${import.meta.env.VITE_API_URL}/payment`, formData
          , {
            headers: {
