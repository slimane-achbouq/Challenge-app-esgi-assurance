<template>
  ...
  <div class="mb-5 flex justify-between">
    ...
    <form class="relative">
      ...
      <input id="app-search" class="form-input w-full pl-9 py-3 focus:border-slate-300" type="search"
             placeholder="Search by claim number" v-model="searchTerm" @input="searchCustomers"/>
          ...
          <div v-show="dropdownOpen"
            ...
            <div ref="dropdown">
              ...
              <ul class="mb-4">
                ...
                <li class="py-1 px-3">
                  <label class="flex items-center">
                    <input type="checkbox" class="form-checkbox" v-model="filters.basicInsurance"/>
                    <span class="text-sm font-medium ml-2">Basic insurance</span>
                  </label>
                </li>
                <li class="py-1 px-3">
                  <label class="flex items-center">
                    <input type="checkbox" class="form-checkbox" v-model="filters.standardInsurance"/>
              ...
              <div class="py-2 px-3 border-t border-slate-200 bg-slate-50">
                <ul class="flex items-center justify-between">
                  ...
                  <li>
                    <button class="btn-xs bg-indigo-500 hover:bg-indigo-600 text-white" @click="applyFilters"
                            @focusout="dropdownOpen = false">Apply
                    </button>
                  </li>
                </ul>
              </div>
        ...
        >
          <div v-show="dropdownOpen1"
            ...
            >
              ...
              >
                <svg class="shrink-0 mr-2 fill-current text-indigo-500" :class="option.id !== selected1 && 'invisible'"
  ...
  <div class="bg-white shadow-lg rounded-sm border border-slate-200 relative">
    ...
    <header class="px-5 py-4">
      <h2 class="font-semibold text-slate-800">All Claims <span
          class="text-slate-400 font-medium">{{ totalResult }}</span></h2>
    </header>
    <div>
  ...
  <div class="mt-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="text-sm text-slate-500 text-center sm:text-left">
        Showing <span class="font-medium text-slate-600">{{ page }}</span> to <span class="font-medium text-slate-600">{{
          lastPage
        }}</span>
        of <span class="font-medium text-slate-600">{{ totalResult }}</span> results
      </div>
    </div>
  </div>
</template>
...
import ClaimsTableItem from './ClaimsTableItem.vue'
