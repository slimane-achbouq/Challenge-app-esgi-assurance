...
export default {
  ...
  setup(props, {emit}) {
    ...
    const clearFilters = () => {
      ...
      claims.value = claimList.value.slice(
    ...
    const selectedDatefilter = (selected) => {
      ...
      if (selected == "4") claims.value = claimList.value;
        ...
        claims.value = claimList.value.filter((claim) => {
          ...
          if (selected == "0" && claimCreationDate.isSame(now, "day"))
            return true;
          if (
              selected == "1" &&
              claimCreationDate.isAfter(sevenDaysAgo) &&
              claimCreationDate.isBefore(now)
          )
            return true;
          if (
              selected == "2" &&
              claimCreationDate.isAfter(oneMonthAgo) &&
              claimCreationDate.isBefore(now)
          )
            return true;
          if (
              selected == "3" &&
              claimCreationDate.isAfter(twelveMonthsAgo) &&
              claimCreationDate.isBefore(now)
          )
            return true;
          if (selected == "4") claims.value = claimList.value;
      ...
      claims.value = claims.value.slice(
          (page.value - 1) * perPage.value,
          page.value * perPage.value
      ); // assign only the corresponding users to the current page
      // get the total users
    ...
    const applyFilters = () => {
      ...
      claims.value = claims.value.filter((claim) => {
        if (filters.value.basicInsurance && claim.coverage == "Basic")
          return true;
        if (filters.value.standardInsurance && claim.coverage == "Standard")
          return true;
        if (filters.value.premiumInsurance && claim.coverage == "Premium")
          return true;
        if (
      ...
      claims.value = claims.value.filter((claim) => {
        if (
            ...
            claim.insuranceType == "Liability"
        )
          return true;
        if (
            filters.value.collisionCoverage &&
            claim.insuranceType == "Collision"
        )
          return true;
        if (
            filters.value.comprehensiveCoverage &&
            claim.insuranceType == "Comprehensive"
        )
          return true;
        if (
            !filters.value.liabilityCoverage &&
    ...
    const fetchClaims = async () => {
      ...
      try {
        response = await axios.get(`${import.meta.env.VITE_API_URL}/claims`, {
          headers: {
      ...
      if (response.data) {
        ...
        claims.value = allUsers.value.slice(
            (page.value - 1) * perPage.value,
            page.value * perPage.value
        ); // assign only the corresponding users to the current page
      }
    ...
    watch(selected, () => {
      selectAll.value =
          claims.value.length === selected.value.length ? true : false;
