    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 relative">
      <header class="px-5 py-4">
        <h2 class="font-semibold text-slate-800">
          All Users <span class="text-slate-400 font-medium"></span>
        </h2>
      </header>
      <div>
        <!-- Table -->
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const fetchUsers = async () => {
      ...
      loading.value = true;
      token = store.getters["auth/token"]; // const response = await axios.get(`${import.meta.env.VITE_API_URL}/users?page=${page.value}`, {
      ...
      if (response.data) {
        ...
        customers.value = allUsers.value.slice(
    ...
    const searchCustomers = async () => {
      ...
      customers.value = JSON.parse(JSON.stringify(customers.value)).filter(
        (customer) =>
