  ...
  <div class="flex h-screen overflow-hidden">
    ...
    <div class="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
      ...
      <main>
        ...
        <Banner type="success" class="mb-4" :open="claimReviewed" v-if="claimReviewed">
          ...
          <div class="max-w-5xl mx-auto flex flex-col ">
            ...
            <div>
              ...
              <div v-if="claim">
                <form>
                  <div class="space-y-3">
                    <h2 class="mt-4 text-xl font-semibold text-slate-800">Claim information</h2>
                    <div class="flex-1">
                      <p><label class="font-medium mb-1" for="card-state">Title</label> : {{ claim.title }}</p>
                    </div>

                    <div class="flex-1">
                      <p><label class="font-medium mb-1" for="card-state">Reason</label> : {{ claim.reason }}</p>
                    </div>

                    <div class="flex-1">
                      <p><label class="font-medium mb-1" for="card-state">Description</label> : {{ claim.description }}
                      </p>
                    </div>
                    ...
                    <h2 class="mt-4 text-xl font-semibold text-slate-800">Your decision</h2>

                    <div class="flex-1">
                      <label class="block text-sm font-medium mb-1" for="card-country">Decision <span
                          class="text-rose-500">*</span></label>
                      <select id="card-country" class="form-select w-full" required v-model="claim.decision">
                      ...
                      <p class="text-xs mt-1 text-rose-500" v-if="errors">{{ errors.decision }}</p>
                    </div>

                    <div class="flex-1">
                      <label class="block text-sm font-medium mb-1" for="card-state">Additional information</label>
                      <textarea id="message" rows="4"
                                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                                placeholder="Explain here..." v-model="claim.additionalInfo"></textarea>
                      <p class="text-xs mt-1 text-rose-500" v-if="errors">{{ errors.additionalInfo }}</p>
                    </div>
                    ...
                    <div class="space-y-3">
                      <div class="text-right">
                        ...
                        <button type="submit"
                                ...
                                @click.prevent="onReviewedClaim" v-if="!claimReviewed">Submit
                        </button>
        ...
        <ModalBlank id="success-modal" :modalOpen="claimReviewed" v-if="claimReviewed">
          <div class="p-5 flex space-x-4">
            ...
            <div>
              <!-- Modal header -->
              <div class="mb-2">
                <div class="text-lg font-semibold text-slate-800">Claim reviewed successfully</div>
              </div>
              <!-- Modal content -->
...
export default {
  ...
  methods: {
    ...
    async onReviewedClaim() {
      ...
      let response = await axios.put(`${import.meta.env.VITE_API_URL}/claims/${this.claim._id}`,
          {
