  ...
  <div class="flex h-screen overflow-hidden">
    ...
    <div class="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
      ...
      <main>
        ...
        <Banner type="success" class="mb-4"  :open="quoteUpdated" v-if="quoteUpdated">
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-4 w-full max-w-9xl mx-auto">
          ...
          <div class="sm:flex sm:justify-between sm:items-center mb-5">
            ...
            <div class="mb-4 sm:mb-0">
              <h1 class="text-2xl md:text-3xl text-slate-800 font-bold"><i class="fa fa-file-contract"></i> Quote Detail ✨</h1>
            </div>
            ...
            <button class="btn border-slate-200 hover:border-slate-300 text-rose-500" @click="modaDeletelOpen=true" v-if="this.quote.insurancePremium==0">
              ...
              <!-- Add account button -->
              <router-link   :to="{ name: 'newcontract', params: { id: quote.id }}">
              <button class="btn bg-indigo-500 hover:bg-indigo-600 text-white">
                <svg class="w-4 h-4 fill-current opacity-50 shrink-0" viewBox="0 0 16 16">
                  <path d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                </svg>
                <span class="hidden xs:block ml-2" v-if="role==='User'">Souscrire</span>
              </button>
              </router-link>
          ...
          <div class="grid grid-cols-12 gap-5">
            ...
            <!-- User detail -->
              ...
              <div class="flex flex-col col-span-full bg-white shadow-lg rounded-sm border border-slate-200">
                <div class="px-5 py-2">
                      <div>
                        <div class="mb-2 "><i class="fas fa-user m-2"></i><strong class="text-1xl text-slate-800">Customer inforamtions</strong> </div>
                      </div>
                  <div class="">
                    <!-- Left side -->
                    <div class="flex items-center mb-4 md:mb-0">
                      <!-- Avatar -->
                      <div class="mr-4">
                        <img class="inline-flex rounded-full" src="@/images/useravatar.png" width="64" height="64" alt="User" />
                      </div>
                      <!-- User info -->
                                      ...
                                      <thead class="text-xs uppercase text-slate-400 bg-slate-50 rounded-sm">
                                        <tr>
                                          <th class="p-2 whitespace-nowrap">
                                            <div class="font-semibold text-left">First Name | Last Name</div>
                                          </th>
                                          ...
                                          <th class="p-2 whitespace-nowrap">
                                            <div class="font-semibold text-left">phoneNumber</div>
                                          </th>
                                          <th class="p-2 whitespace-nowrap">
                                        ...
                                        <tr>
                                          <td class="p-2 whitespace-nowrap ">
                                            <div class="flex items-center">
                                              <div>
                                                <div class="text-slate-800 uppercase"> {{user.firstname}} {{user.lastname}}</div>
                                              </div>
                                            </div>
                                          </td>
                                          <td class="p-2 whitespace-nowrap">
                                            <div class="font-normal text-left">{{user.email}}</div>
                                          </td>
                                          <td class="p-2 whitespace-nowrap">
                                            <div class="font-normal text-left">{{user.phoneNumber}}</div>
                                          </td>
                                          <td class="p-2 whitespace-nowrap">
                                            <div class="font-normal text-left">{{user.adresse}} {{user.city}}</div>
                                          </td>                              
                                          <td class="p-2 whitespace-nowrap">
                                            <div class="text-left ">{{calculateAge(user.age)}} yo</div>
                                          </td>
                                          <td class="p-2 whitespace-nowrap">
                                            <div class="text-left text-emerald-500"><span class=" bg-emerald-100 text-emerald-600 font-medium rounded-full text-center px-2.5 py-1" v-if="user.isValide">Actif</span><span class="bg-amber-100 text-amber-600 font-medium rounded-full text-center px-2.5 py-1" v-if="!user.isValide">Inactif</span></div>
                                          </td>
                                        </tr>
                                      </tbody>
                                    </table>
          ...
          <!-- quote info -->
                ...
                <div class="px-10  divide-y divide-slate-100 space-y-3">
                      ...
                      <div>
                        <div class="flex justify-between text-sm m-3">
                          ...
                          <div class="  "> <span class=" bg-emerald-100 text-emerald-600 font-medium rounded-full text-center px-2.5 py-1" v-if="quote.insuranceId">subscribed</span><span class="bg-amber-100 text-amber-600 font-medium rounded-full text-center px-2.5 py-1" v-if="!quote.insuranceId">unsubscribed</span> </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <div class="flex justify-between text-sm m-3">
                          ...
                          <div class="">{{processDate(quote.coverageStartDate)}}</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-3">
                    ...
                    <div class="divide-y divide-slate-300 m-3">
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          ...
                          <div class="text-slate-400 italic">{{quote.vehicle.vehicleType}}</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          <div class="text-slate-300">Brand</div>
                          <div class="text-slate-400 italic">{{quote.vehicle.brand}}</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          <div class="text-slate-300">Model</div>
                          <div class="text-slate-400 italic">{{quote.vehicle.model}}</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          <div class="text-slate-300">Horsepower</div>
                          <div class="text-slate-400 italic">{{quote.vehicle.horsepower}} HP</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          <div class="text-slate-300">License Plate</div>
                          <div class="text-slate-400 italic">{{quote.vehicle.licensePlate}}</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          <div class="text-slate-300">License Obtained Date</div>
                          <div class="text-slate-400 italic">{{quote.vehicle.licenseObtainedDate}}</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          <div class="text-slate-300">Vehicle Circulation Date</div>
                          <div class="text-slate-400 italic">{{quote.vehicle.vehicleCirculationDate}}</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          <div class="text-slate-300">Registration Card Holder</div>
                          <div class="text-slate-400 italic">{{quote.vehicle.registrationCardHolder}}</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          <div class="text-slate-300">registrationCardDate</div>
                          <div class="text-slate-400 italic">{{quote.vehicle.registrationCardDate}}</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          <div class="text-slate-300">Purshace type</div>
                          <div class="text-slate-400 italic">{{quote.vehicle.purchaseMode}}</div>
                        </div>
                      </div>
                      <div>
                        <div class="flex justify-between text-sm m-1">
                          <div class="text-slate-300">Annual Mileage</div>
                          <div class="text-slate-400 italic">{{quote.vehicle.annualMileage}}</div>
                        </div>
                      </div>
            ...
            <div class="col-span-full bg-white shadow-lg rounded-sm border border-slate-200 mx-8">
                            ...
                            <div class="md:flex justify-between items-center space-y-4 md:space-y-0 space-x-2">
                                ...
                                <div class="flex items-start space-x-3 md:space-x-4">
                                    <div class="w-9 h-9 shrink-0 mt-1">
                                        <img @click="downloadFile" class="w-9 h-9 rounded-full"  src="@/images/pdf.png" width="36" height="36"  />
                                    </div>
                                    <div>
                                        Carte Grise file : 
            ...
            <ModalBasic id="danger-modal" :modalOpen="modaDeletelOpen" >
                      <div class="p-5 flex w-full space-x-4">
                        ...
                        <div>
                          <!-- Modal header -->
                          <div class="mb-2">
                            <div class="text-lg font-semibold text-slate-800">Delete Quote?</div>
                          </div>
                          <!-- Modal content -->
...
export default {
  ...
  data() {
    ...
    methods:{
      ...
      async  deleteItem(){
        ...
        try {const response = await axios.delete(`${import.meta.env.VITE_API_URL}/quotes/${quote.id}` , {
              headers: {
  ...
  async created() {
        ...
        const token = this.$store.getters["auth/token"]
        // const response = await axios.get(`${import.meta.env.VITE_API_URL}/users?page=${page.value}`, {
          const response = await axios.get(`${import.meta.env.VITE_API_URL}/quotes/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
        ...
        const response1 = await axios.get(`http://localhost:3000/getoneuser/${this.quote.userId}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
