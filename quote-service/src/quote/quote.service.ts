...
export class QuoteService {
  ...
  async createQuote(quoteDto: CreateQuoteDto): Promise<Quote> {
    ...
    if (!vehicle) {
      this.logger.error(
        'createQuote error : Vehicle not found => ' + JSON.stringify(quoteDto),
        'error',
      );
  ...
  async getQuoteById(id: string): Promise<Quote> {
    ...
    try {
      ...
      this.logger.error(
        'getQuoteById => Error Quote ID ' + id + ' not found',
        'error',
      );
  ...
  async updateQuote(id: string, quoteDto: UpdateQuoteDto): Promise<Quote> {
    this.logger.log(
      'debug',
      'updateQuote => insuranceType ' + quoteDto.insuranceType,
    );
    const fieldName = 'id';
  ...
  async deleteQuote(id: string): Promise<void> {
    ...
    this.logger.error(
      'deleteQuote => Error Quote ID ' + id + ' not found',
      'error',
    );
  }
}
